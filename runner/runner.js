...
io.sockets.on('connection', function (socket, pseudo) {
    ...
    socket.on('proto', function(request) {
		console.log( "[runner] receive [proto] : "+request);
		if( request == 'test' )
			socket.emit('proto', 'OK');
    });
    
    socket.on('log', function(request) {
		console.log( "[runner] receive [log] : "+request);
    ...
    socket.on('message', function (message) {
        ...
        socket.get('pseudo', function (error, pseudo) {
            console.log('[runner] '+pseudo + ' me parle ! Il me dit : ' + message);
        });
    }); 
    ...
    socket.on('push', function (request) {
        ...
        writeData( request );
        for( e in request )
			console.log( '[runner] push '+e+' : '+request[e] );
    }); 
    ...
    socket.on('save', function (request) { // save la BDD complete
        ...
        //console.log( "push : "+request);
        saveBdd( request );
        console.log( "[runner] Save BDD OK" );
    }); 
});
